    ...
    private async Task ProcessCompletedDonation(DonationCompletedMessage message)
{
    ...
    try
    {
        ...
        {
            _logger.LogWarning($"DonationActor: Donation status is not SUCCESS (it's {donation.PaymentStatus}). Checking if need to update.");
    ...
    private async Task UpdateDonationStatistics(decimal amount)
{
    ...
    try
    {
        ...
        try
        {
            ...
            _logger.LogInformation($"DonationActor: Successfully updated statistics. New values - Total: {stats.TotalDonations}, Donors: {stats.TotalDonors}, Meals: {stats.MealsServed}");
        }
        catch (Exception ex)
        {
